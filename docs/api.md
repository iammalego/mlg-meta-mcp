...
This project currently exposes **20 MCP tools**. The tool registry in `src/tools/index.ts` is the authoritative source for names, descriptions, and argument validation.
...
| `createAd` | Create an ad with a structured creative payload. | `adSetId`, `name`, `creative`, `status?`                      |

The `creative` object supports `title`, `body`, `imageUrl`, `linkUrl`, and `callToAction`.

## Insights tools

| Tool                | Purpose                                                                       | Key arguments                                                                              |
| ------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `getInsights`       | Fetch aggregated performance metrics for an account, campaign, ad set, or ad. | `objectId`, `level` (`account`/`campaign`/`adset`/`ad`), `datePreset?`, `timeRange?`       |
...
- `timeRange` uses `{ since, until }` in `YYYY-MM-DD` format.
- If `timeRange` is present, it takes precedence over `datePreset`.
- Supported presets are `today`, `yesterday`, `last_7d`, `last_30d`, `this_month`, and `last_month`.
...
| Tool                    | Purpose                                                            | Key arguments                                                                 |
| ----------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| `cloneCampaign`         | Clone a campaign structure, optionally copying ad sets and ads.    | `sourceCampaignId`, `newName`, `copyAdSets?`, `copyAds?`, `budgetAdjustment?` |
| `cloneAdSet`            | Clone an ad set into a target campaign.                            | `sourceAdSetId`, `targetCampaignId`, `newName?`, `copyAds?`                   |
